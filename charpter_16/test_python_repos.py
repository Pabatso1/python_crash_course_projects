...
def get_project_data(repo_dicts):
    """Return data needed for each project in visiualization."""
...
def make_visualization(repo_links, stars, labels):
    """Generate visualization of most commented articles."""
    data = [{
        'type': 'bar',
        'x': repo_links,
        'y': stars,
    ...
    my_layout = {
        'title': 'Most_Starred Python Program on Github',
        'titlefont': {'size': 28},
        'xaxis': {
            'title': 'Repository',
            'titlefont': {'size': 24},
    ...
    offline.plot(fig, filename='ppython_repos.html')

    if __name__ == '__main__':
        r = get_response()
        repo_dicts = get_repo_dicts(r)
        repo_links, stars, labels = get_project_data(repo_dicts)
        make_visuaization(repo_links, stars, labels)